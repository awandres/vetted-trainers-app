    ...
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          scrolled || mobileMenuOpen ? "bg-[#0f0f0f]" : "bg-[#181818]/80 backdrop-blur-sm"
        }`}
      >
        <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
              ...
              {navLinks.map((link) => (
                <Link
                  ...
                  className={`font-medium transition-colors relative group ${
                    isActive(link.href) ? "text-[#50BFF4]" : "text-gray-300 hover:text-white"
                  }`}
                >
                  {link.label}
                  <span
            ...
            >
              ...
              <span className={`block w-6 h-0.5 bg-white transition-all duration-300 mt-1.5 ${mobileMenuOpen ? "opacity-0" : ""}`} />
              <span className={`block w-6 h-0.5 bg-white transition-all duration-300 mt-1.5 ${mobileMenuOpen ? "-rotate-45 -translate-y-2.5" : ""}`} />
            </button>
          </div>
        </nav>
      </header>
                ...
                <li key={link.href}>
                  <Link
                    ...
                    className={`block py-4 text-2xl font-bold transition-colors border-b border-white/10 ${
                      isActive(link.href) ? "text-[#50BFF4]" : "text-white"
                    }`}
                  >
                    {link.label}
                  </Link>
